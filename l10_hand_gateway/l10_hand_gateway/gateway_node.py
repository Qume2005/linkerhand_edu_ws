...
广播:
  /l10_gateway/camera              Float32MultiArray [nx, ny, nz, distance]
...
class L10HandGatewayNode(Node):
    def __init__(self):
        ...
        self.get_logger().info(
            ...
            f"Broadcasting 7 topics, subscribing 5 cmd + 1 backend.")
    ...
    def _on_cmd_cp(self, msg):
        ...
        dof = ik_control_points(targets, self._target_dof)
        self._target_dof = [max(0.0, min(255.0, v)) for v in dof]
        self._broadcast_target()
        self._forward_to_backend()
    ...
    def _on_cmd_camera(self, msg):
        if len(msg.data) < 4:
            return
        self._camera = [float(v) for v in msg.data[:4]]
        self._broadcast_camera()
