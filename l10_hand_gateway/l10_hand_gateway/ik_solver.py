...
def ik_control_points(target_positions_5, initial_dof_10):
    """
    ...
    for finger_idx in range(5):
        ...
        if n == 1:
            _ik_solve_1dof(arc_10, dofs[0], body_idx, target)
        else:
            _ik_solve_ndof(arc_10, dofs, n, body_idx, target)

    return arc_to_range_l10_right(arc_10)


def _ik_solve_1dof(arc, dof, body_idx, target):
    ...
    for _ in range(12):
        ...
        dc = _dist3d_sq(pc, target)

        arc[dof] = d
        pd = _fk_fingertip(arc, body_idx)
        dd = _dist3d_sq(pd, target)
...
def _ik_solve_ndof(arc, dofs, n, body_idx, target):
